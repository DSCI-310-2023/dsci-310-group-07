...
Project Authors (Original): STAT 301 Group 36: Henry Zhang, Moira Renata, Ning Wang, Paige Wills, Xinrui Wang

```{r setup, message = FALSE, warning=FALSE, include=FALSE}
library(leaps)
library(tidyverse)
library(here)
library(knitr) # TODO: add this to Dockerfile
# Source data
...
In this section, we load and clean the data. Note that the last column `?` with all values are is removed.


```{r read data, message=FALSE, warning=FALSE}
# Labelling all the columns
automobile <- get_automobile()
head(automobile) %>% kable(digits = 3)
```
...
summary(automobile) %>% kable(digit = 4)
```
...
#names<-colnames(automobile[,-ncol(automobile)])
#create a summary table of the predictor variables with the top 8 highest R^2 values
#df_sqr<-cbind(r_sqr,names)%>%as.data.frame()
#df_sqr$r_sqr<-as.numeric(df_sqr$r_sqr)
#kable(top8 <- head(df_sqr%>%arrange(desc(df_sqr$r_sqr)),8),digits = 4)
...
# a list of the top  8 variables with the highest coefficient of determination
nms <- showR2(automobile, 8)%>%select(names)%>%pull()
nms
