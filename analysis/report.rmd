...
```{r remove unkown col, message=FALSE, warning=FALSE}

##process df using processdata function and name it automobile
automobile<-processdata(df)
#number of rows in the dataframe
nrow(automobile)
#Check that the last column is now the response variable
colnames(automobile[,ncol(automobile)])
```

From above, we can see that there are 45 rows that contain NA values; this is calculated by 205-160.

```{r print tidy data, message=FALSE, warning=FALSE}
